...
# CRT glow ver
CRT_GREEN_GLOW = "#66FFAA"
CRT_RED_GLOW = "#FF8888"
CRT_YELLOW_GLOW = "#FFFF99"
...
def draw_crt_grid(canvas, offset=0):
    w, h = canvas.winfo_width(), canvas.winfo_height()
    if w < 10 or h < 10: return
    grid_spacing_x = max(1, w // 10)
    grid_spacing_y = max(1, h // 5)

    # Horizontal scrolling effect
    x_offset = offset % grid_spacing_x
    for x in range(-x_offset, w, grid_spacing_x):
        canvas.create_line(x, 0, x, h, fill=CRT_GRID)
    for y in range(0, h, grid_spacing_y):
        canvas.create_line(0, y, w, y, fill=CRT_GRID)

def draw_crt_line(canvas, data, max_value, line_color, width=2, glow=True):
    w, h = canvas.winfo_width(), canvas.winfo_height()
    if len(data) < 2 or w < 10 or h < 10:
        return

    step = w / MAX_POINTS
    pts = [(i * step, h - (val / max(1e-6, max_value)) * h) for i, val in enumerate(data)]

    # Simulate glow by drawing outer lines first
    if glow:
        glow_layers = [
            {"color": line_color, "width": width + 4},
            {"color": line_color, "width": width + 2},
        ]
        for layer in glow_layers:
            for i in range(len(pts) - 1):
                canvas.create_line(
                    ...
                    fill=layer["color"],
                    width=layer["width"],
                    stipple="gray50"  # Simulates transparency
                )
...
def draw_metric(canvas, series, max_value, color=CRT_GREEN):
    canvas.delete("all")
    draw_crt_grid(canvas, offset=frame_count)
    smoothed_series = smooth_data(series)
    draw_crt_line(canvas, smoothed_series, max_value, color, glow=True)

def draw_dual_io(canvas, read_hist, write_hist):
    canvas.delete("all")
    draw_crt_grid(canvas, offset=frame_count)
    max_io = max(read_hist + write_hist + [1])
    smoothed_read = smooth_data(read_hist)
    smoothed_write = smooth_data(write_hist)
    draw_crt_line(canvas, smoothed_read, max_io, "white", glow=True)
    draw_crt_line(canvas, smoothed_write, max_io, CRT_GREEN, glow=True)
...
def build_metric_frame(parent, title, maxval=100, graph_height=GRAPH_HEIGHT):
    ...
    cvs = tb.Canvas(f, height=graph_height, background="white", highlightthickness=0)
...
for metric in metric_list:
    ...
    if metric.get("io", False):
        ...
        io_write_lbl = tb.Label(f, text="WRITE: ...", anchor="w", font=("Courier", 8, "bold"), foreground=CRT_GREEN)
        io_write_lbl.pack(fill=X, padx=4, pady=(2,0))
        io_write_bar_style = f"IOWrite.Horizontal.TProgressbar"
        style.configure(io_write_bar_style, troughcolor="black", background=CRT_GREEN, thickness=PROGRESS_THICKNESS)
        ...
        style.configure(io_read_bar_style, troughcolor="black", background="white", thickness=PROGRESS_THICKNESS)
        ...
        f = tb.Labelframe(root, text="System Info & Local Time", bootstyle="white")
        f.grid(row=row, column=col, columnspan=colspan, sticky="nsew", padx=4, pady=4)
...
def update_stats():
    ...
    for key, val in usage_values.items():
        ...
        if key == "RAM":
            ...
            lbl.config(text=f"RAM Utilized: {ram_info['used']} GB used | {ram_info['available']} GB Remaining | ({val:.1f}%)")
